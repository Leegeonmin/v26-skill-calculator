create or replace function public.cleanup_tool_usage_events(
...
as $$
declare
  v_retention_days integer := greatest(coalesce(p_retention_days, 3), 1);
...
revoke all on function public.cleanup_tool_usage_events(integer) from anon, authenticated;
...
begin
  if exists (
    select 1
    from cron.job
    where jobname = 'cleanup-tool-usage-events'
  ) then
...
select cron.schedule(
  'cleanup-tool-usage-events',
  '15 18 * * *',
  $$select public.cleanup_tool_usage_events(3);$$
);
