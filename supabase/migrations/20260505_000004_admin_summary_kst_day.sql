...
declare
  ...
  v_seven_day_start timestamptz := v_today_start - interval '6 days';
  v_thirty_day_start timestamptz := v_today_start - interval '29 days';
...
begin
  ...
  with latest_events as (
    ...
    where e.created_at >= v_thirty_day_start
  ...
  summary as (
    select
      ...
      (select count(*)::bigint from latest_events le7 where le7.created_at >= v_seven_day_start) as seven_day_events,
      (select count(*)::bigint from latest_events le30 where le30.created_at >= v_thirty_day_start) as thirty_day_events,
