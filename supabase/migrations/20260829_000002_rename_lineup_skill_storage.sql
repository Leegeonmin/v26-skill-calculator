alter table if exists public.skill_ocr_public_uploads
  rename to lineup_skill_uploads;

create index if not exists idx_lineup_skill_uploads_user_updated_at
  on public.lineup_skill_uploads (user_id, updated_at desc);

create or replace function public.lineup_skill_save_upload(
  p_role text,
  p_image_name text,
  p_request_id text,
...
declare
  ...
  v_upload public.lineup_skill_uploads;
begin
  ...
  if p_role not in ('hitter', 'pitcher') then
    raise exception 'INVALID_LINEUP_SKILL_ROLE';
  end if;
  ...
  if v_player_count > 9 then
    raise exception 'LINEUP_SKILL_PLAYER_LIMIT_EXCEEDED';
  end if;

  insert into public.lineup_skill_uploads (
    user_id,
    role,
    is_saved,
    image_name,
...
create or replace function public.lineup_skill_finalize_upload(
  p_upload_id uuid,
  p_role text,
  p_image_name text,
  p_request_id text,
...
begin
  ...
  update public.lineup_skill_uploads upload_row
     set role = p_role,
         is_saved = true,
  ...
  if v_upload.id is null then
    raise exception 'LINEUP_SKILL_UPLOAD_NOT_FOUND';
  end if;
...
create or replace function public.lineup_skill_list_uploads(
...
begin
  ...
  from public.lineup_skill_uploads upload_row
  where upload_row.user_id = v_user_id
  order by upload_row.updated_at desc, upload_row.created_at desc
  limit least(greatest(coalesce(p_limit, 20), 1), 100);
end;
$$;

create or replace function public.lineup_skill_delete_upload(
  p_upload_id uuid
)
returns void
language plpgsql
...
begin
  ...
  delete from public.lineup_skill_uploads upload_row
   where upload_row.id = p_upload_id
     and upload_row.user_id = v_user_id
...
grant execute on function public.lineup_skill_save_upload(text, text, text, jsonb, jsonb, numeric, numeric) to authenticated;
grant execute on function public.lineup_skill_finalize_upload(uuid, text, text, text, jsonb, jsonb, numeric, numeric) to authenticated;
grant execute on function public.lineup_skill_list_uploads(integer) to authenticated;
grant execute on function public.lineup_skill_delete_upload(uuid) to authenticated;
...
drop function if exists public.skill_ocr_delete_public_upload(uuid);
