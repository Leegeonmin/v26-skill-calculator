...
begin
  ...
  select
    ...
    count(*) filter (
      where e.tool in ('ocr_lineup_recognize', 'ocr_skill_compare_recognize')
    )::bigint as ocr_total_requests,
    count(*) filter (where e.tool = 'ocr_lineup_recognize')::bigint as ocr_lineup_requests,
    count(*) filter (where e.tool = 'ocr_skill_compare_recognize')::bigint as ocr_skill_compare_requests,
    count(*) filter (where e.tool = 'ocr_lineup_recognize' and e.mode = 'hitter')::bigint as ocr_hitter_requests,
    count(*) filter (where e.tool = 'ocr_lineup_recognize' and e.mode = 'pitcher')::bigint as ocr_pitcher_requests,
    (select count(*)::bigint from public.skill_ocr_uploads) as ocr_saved_uploads,
    (select count(*)::bigint from public.skill_ocr_uploads where role = 'hitter') as ocr_saved_hitter_uploads,
    (select count(*)::bigint from public.skill_ocr_uploads where role = 'pitcher') as ocr_saved_pitcher_uploads,
    jsonb_build_array(
      jsonb_build_object(
        'label', '라인업 OCR - 투수',
        'request_count', count(*) filter (where e.tool = 'ocr_lineup_recognize' and e.mode = 'pitcher'),
        'unique_sessions', count(distinct nullif(e.metadata->>'session_id', '')) filter (
          where e.tool = 'ocr_lineup_recognize' and e.mode = 'pitcher'
        ),
        'saved_count', (select count(*)::bigint from public.skill_ocr_uploads where role = 'pitcher'),
        'last_seen_at', max(e.created_at) filter (where e.tool = 'ocr_lineup_recognize' and e.mode = 'pitcher')
      ),
      jsonb_build_object(
        'label', '라인업 OCR - 타자',
        'request_count', count(*) filter (where e.tool = 'ocr_lineup_recognize' and e.mode = 'hitter'),
        'unique_sessions', count(distinct nullif(e.metadata->>'session_id', '')) filter (
          where e.tool = 'ocr_lineup_recognize' and e.mode = 'hitter'
        ),
        'saved_count', (select count(*)::bigint from public.skill_ocr_uploads where role = 'hitter'),
        'last_seen_at', max(e.created_at) filter (where e.tool = 'ocr_lineup_recognize' and e.mode = 'hitter')
      ),
      jsonb_build_object(
        'label', '스킬 비교 OCR',
        'request_count', count(*) filter (where e.tool = 'ocr_skill_compare_recognize'),
        'unique_sessions', count(distinct nullif(e.metadata->>'session_id', '')) filter (
          where e.tool = 'ocr_skill_compare_recognize'
        ),
        'saved_count', 0,
        'last_seen_at', max(e.created_at) filter (where e.tool = 'ocr_skill_compare_recognize')
      )
    ) as ocr_breakdown,
    coalesce((
      select jsonb_agg(
