...
returns table (
  quota_role text,
  quota_used boolean,
  quota_used_at timestamptz,
  quota_week_start_date date
)
language plpgsql
security definer
...
begin
  ...
  select
    ...
    quota_rows.quota_used,
    quota_rows.quota_used_at,
    quota_rows.quota_week_start_date
