    ...
    sections: [
      ...
      },
      {
        ...
        body: (
          <>
            <p>
              희귀도는 현재 카드에서 나올 수 있는 스킬 조합을 펼친 뒤, 같은 계열 스킬이 한 조합에
              겹치지 않도록 제외하고 현재 총점 이상이 나올 확률을 더해 구합니다. 골든글러브는
              6-6-6, 나머지 카드는 6-5-5를 기준으로 분포를 계산합니다. 기대 횟수는 1을 해당
              확률로 나눈 값이라, 확률이 5%면 평균적으로 20회 정도라는 뜻입니다.
            </p>
            <ul>
      ...
      },
      {
        ...
        body: (
          <p>
            ...
            내용은 공지사항에 남기고, 계산기와 OCR이 서로 다른 점수표를 쓰지 않도록 같은 데이터로
            함께 업데이트합니다.
          </p>
        ),
      },
  ...
  simulatorGuide: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: (
          <ul>
            <li>1회 실행은 가능한 조합을 한 번 뽑아 점수와 등급을 바로 보여줍니다.</li>
            <li>
              자동 롤은 B, A, S, SS, SR+ 중 목표를 정하고 그 등급 이상이 나올 때까지 반복합니다.
            </li>
            <li>
              한 번 누를 때 최대 5,000회까지 확인하며, 목표에 도달하면 그 시점에서 멈춥니다.
            </li>
          </ul>
        ),
      },
      ...
      },
      {
        ...
        body: (
          <p>
            임팩트 변경 시뮬은 사용자가 고른 1번 스킬을 그대로 두고 2번과 3번만 다시 뽑습니다. 고정한
            스킬과 같은 계열은 후보에서 빠지고, 나머지 두 칸이 모두 메이저가 될 때까지 반복합니다.
            한 번 실행할 때 최대 100,000회까지 시도하며 성공하면 실제 반복 횟수와 나온 조합을
            보여줍니다.
          </p>
        ),
      },
    ...
    sections: [
      {
        ...
        body: (
          <>
            <p>
              PNG, JPG, JPEG, WEBP 파일을 올릴 수 있습니다. 모바일 라인업의 한눈에 보기 화면처럼
              선수명과 스킬 3개, 레벨 배지가 한 화면에 보이는 원본 캡처가 가장 적합합니다. 메신저로
              여러 번 전송한 이미지나 확대 후 다시 저장한 이미지는 작은 레벨 숫자가 뭉개질 수 있습니다.
            </p>
      ...
      },
      {
        ...
        body: (
          <ol>
            <li>Google 로그인 후 투수 또는 타자 업로드를 선택합니다.</li>
            <li>이미지에서 선수 행과 스킬 칸을 찾고 선수명, 스킬명, 레벨 후보를 읽습니다.</li>
            <li>인식 결과를 사이트의 스킬 목록과 연결해 선수별 점수와 라인업 평균을 계산합니다.</li>
            <li>사용자가 카드 타입, 포지션, 스킬, 레벨을 직접 확인하고 틀린 값을 고칩니다.</li>
            <li>필요한 선수만 선택해 최종 결과를 저장하거나 텍스트로 복사합니다.</li>
          </ol>
        ),
      },
      {
      ...
      },
      {
        ...
        body: (
          <p>
            OCR은 정답을 확정하는 기능이 아니라 입력을 빠르게 채우는 기능입니다. 카드 타입, 투수 보직,
            스킬명, 레벨 또는 선수 선택 여부를 바꾸면 현재 선택된 선수들의 총점과 평균을 계산기와 같은
            점수표로 다시 계산합니다. 저장 전에는 이미지와 결과를 한 줄씩 비교하는 편이 안전합니다.
          </p>
        ),
      },
      {
        ...
        body: (
          <p>
            공개 OCR은 Google 로그인이 필요하며 현재 타자와 투수를 각각 주 1회 사용할 수 있습니다.
            업로드 후에는 파일명, OCR 응답, 사용자가 고친 선수·스킬 선택값, 총점과 평균점수가 계정의
            스냅샷으로 저장됩니다. 화면에는 최근 타자·투수 결과를 다시 열거나 복사할 수 있도록
            표시합니다.
          </p>
        ),
      },
      {
        ...
        body: (
          <p>
            인식 직후에는 아직 최종 저장하지 않은 스냅샷이 만들어집니다. 사용자는 검수 화면에서 이
            스냅샷을 다시 열거나 삭제할 수 있습니다. 최종 저장한 기록이나 계정 관련 정보의 삭제가
            필요하면 <a href="/contact">문의 페이지</a> 또는 이메일로 요청할 수 있습니다.
          </p>
        ),
      },
      {
      ...
      },
      {
        ...
        body: (
          <p>
            화면 테마, 해상도, 이미지 압축, 새 스킬 아이콘에 따라 인식률이 달라질 수 있습니다. OCR
            결과만 보고 게임 내 판단을 확정하지 말고, 매칭 실패나 반복 오류가 있으면 원본 이미지와
            함께 제보해주세요.
          </p>
        ),
      },
    ...
    sections: [
      {
        ...
        body: (
          <ul>
            ...
            <li>OCR 사용 시 파일명, 인식 결과, 사용자가 수정한 선수·스킬 정보와 계산 점수</li>
